function translateHeadings(notes) {
  return notes
    ...
    .replace(/^### ⚠️ Breaking Changes$/gm, '### ⚠️ 破壊的変更 / Breaking Changes');
}
...
module.exports = {
  generateNotes: async (pluginConfig, context) => {
    const englishNotes = (context.nextRelease && context.nextRelease.notes || '').trim();
    ...
    if (hasBreakingChanges) {
      sections.push('**Breaking Changes / 破壊的変更**: 重要な変更点がありますので必ずご確認ください。');
    }
    ...
    if (contributors.length > 0) {
      ...
      sections.push(contributors.map((contributor) => `- ${contributor}`).join('\n'));
    }
